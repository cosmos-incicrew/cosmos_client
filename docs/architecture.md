...
화면은 사용자 여정 기준으로 묶고, **서버 모듈과의 대응**을 함께 표기한다. 서버 도메인 용어
(BSTI·성분비교·추천)를 프론트에서도 그대로 쓴다 — 팀 소통과 API 매핑을 위해서다.

| 기능(feature) | 화면 | 대응 서버 모듈 | URL prefix |
|---|---|---|---|
| `auth` | 로그인 | (Supabase Auth 직접) | — |
| `ingredient_search` | 제품명·성분명 검색 | `ingredient_search` | `/api/v1/ingredients` |
| `ingredient_detail` | 개별 성분 해설·주의사항 | `ingredient_detail` | `/api/v1/ingredients` |
| `product_compare` | 2개 제품 교차 조회 | `product_compare` | `/api/v1/products` |
| `bsti` | BSTI 16타입 검사·결과 | `bsti` | `/api/v1/bsti` |
| `recommendation` | 성분 추천 | `recommendation` | `/api/v1/recommendations` |
| `profile` | 마이페이지 | (여러 모듈 조합) | — |

> **현재 상태:** 스캐폴드에는 `auth`·`home`·`search`·`product`·`profile`만 있다.
> `bsti`·`product_compare`·`recommendation` 전용 화면은 아직 없다. 서버 도메인에 맞춰
> 화면 작업 시 이 표를 목표 구조로 삼는다. 각 기능은 담당자 1명의 작업 영역으로 나눠
